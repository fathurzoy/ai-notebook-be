...
type INotebookRepository interface {
	...
	NullifyParrentById(ctx context.Context, parentId uuid.UUID) error
	UpdateParentId(ctx context.Context, uuid uuid.UUID, parentId uuid.UUID) error
}
...
func (n *notebookRepository) GetAll(ctx context.Context) ([]*entity.Notebook, error) {
	...
	for rows.Next() {
		...
		err := rows.Scan(&notebook.Id, &notebook.Name, &notebook.ParentId, &notebook.CreatedAt, &notebook.UpdatedAt, &notebook.DeletedAt, &notebook.IsDeleted)
		if err != nil {
			return nil, err
		}
...
func (n *notebookRepository) Create(ctx context.Context, notebook *entity.Notebook) error {
	...
	_, err := n.db.Exec(
		...
		notebook.Id, notebook.Name, notebook.ParentId, notebook.CreatedAt, notebook.UpdatedAt, notebook.DeletedAt, notebook.IsDeleted,
	)
...
func (n *notebookRepository) GetById(ctx context.Context, notebook uuid.UUID) (*entity.Notebook, error) {

	row := n.db.QueryRow(
		ctx,
		`SELECT id, name, parent_id, created_at, updated_at, deleted_at, is_deleted FROM notebook WHERE id = $1`,
		notebook,
	)
	var notebookEntity entity.Notebook
	err := row.Scan(
		&notebookEntity.Id, &notebookEntity.Name, &notebookEntity.ParentId, &notebookEntity.CreatedAt, &notebookEntity.UpdatedAt, &notebookEntity.DeletedAt, &notebookEntity.IsDeleted,
	)
	if err != nil {
...
func (n *notebookRepository) Update(ctx context.Context, notebook *entity.Notebook) error {
	...
	_, err := n.db.Exec(
		...
		notebook.Name, notebook.ParentId, notebook.UpdatedAt, notebook.Id,
	)
...
func (n *notebookRepository) DeleteById(ctx context.Context, id uuid.UUID) error {
	...
	_, err := n.db.Exec(
		ctx,
		`UPDATE notebook SET is_deleted = $1, deleted_at = $2 WHERE id = $3`,
		true, time.Now(), id,
	)
...
func (n *notebookRepository) NullifyParrentById(ctx context.Context, parentId uuid.UUID) error {
...
func (n *notebookRepository) UpdateParentId(ctx context.Context, uuid uuid.UUID, parentId uuid.UUID) error {

	_, err := n.db.Exec(
		ctx,
		`UPDATE notebook SET parent_id = $1, updated_at = $2 WHERE id = $3`,
		parentId, time.Now(), uuid,
	)
